##########################################
# the last main.py(save the plot data)
# run the ROI data on the transfer learning model
##########################################
...
from network.Dialated_AC_in_DenseNet7_new import DenseNet
from lib.logger import Logger, savefig
from lib.misc import AverageMeter, mkdir_p
import dataset_nodule as dataset
from lib import utils_acc_auc_new_2 as utils
import os
...
################# Datasets
parser.add_argument('-d', '--data', default="/data2/lwy/dataset/Nodule/dataset3_scaled_z12/0_255/", type=str)
# parser.add_argument('-d', '--data', default="/data2/lwy/dataset/dataset11/", type=str)
...
################# Checkpoints
parser.add_argument('-c', '--checkpoint', default="/data2/lwy/projevt/Lung/9.23/VGG_egfr9_Dialated_AC_in_Dense_K_fold_new/output/11_12_output_ROC/model9_densenet_11_12_ROI_contrast",
...
def train_val(model, opti_model_parameters, train_loader1, test_loader1, fold_index):
    best_acc = 0
    best_auc = 0

    state = {k: v for k, v in args._get_kwargs()} # To dictionary
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.SGD(opti_model_parameters, lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    ...
    for epoch in range(0, args.epochs):
        ...
        test_loss, test_acc, test_auc, test_gt_list_array, test_pred_list_array = utils.test(test_loader1, model, criterion, use_cuda)
...
def main1(train_loader1, test_loader1):
    ###########    model
    model1 = product_the_pretrained_densenet_model()
    opti_model_parameters = freeze_target_layer(model1)
    model1.cuda()
    ...
    ############### Train
    best_acc1, best_auc1 = train_val(model1,opti_model_parameters, train_loader1, test_loader1, '1')
...
def main2(train_loader2, test_loader2):
    ###########    model
    model2 = product_the_pretrained_densenet_model()
    opti_model_parameters = freeze_target_layer(model2)
    model2.cuda()
    ...
    ############### Train
    best_acc2, best_auc2 = train_val(model2,opti_model_parameters, train_loader2, test_loader2, '2')
...
def main3(train_loader3, test_loader3):
    ###########    model
    model3 = product_the_pretrained_densenet_model()
    opti_model_parameters = freeze_target_layer(model3)
    model3.cuda()
    ...
    ############### Train
    best_acc3, best_auc3 = train_val(model3, opti_model_parameters, train_loader3, test_loader3, '3')
...
def main4(train_loader4, test_loader4):
    ###########    model
    model4 = product_the_pretrained_densenet_model()
    opti_model_parameters = freeze_target_layer(model4)
    model4.cuda()
    ...
    ############### Train
    best_acc4, best_auc4 = train_val(model4, opti_model_parameters, train_loader4, test_loader4, '4')
...
def main5(train_loader5, test_loader5):
    ###########    model
    model5 = product_the_pretrained_densenet_model()
    opti_model_parameters = freeze_target_layer(model5)
    model5.cuda()
    ...
    ############### Train
    best_acc5, best_auc5 = train_val(model5, opti_model_parameters, train_loader5, test_loader5, '5')
