...
import lib.dataset_new_CT as dataset_new
...
def test(val_loader, model, criterion, use_cuda, batch_size):
    ...
    pred_score_all, gt_all, name_all = [], [], []
    ...
    return (losses.avg, top1.avg, auc, np.array(gt_all), np.array(pred_score_all), names)
