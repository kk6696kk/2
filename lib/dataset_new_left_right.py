...
TRANSFORM = transforms.Compose([transforms.Resize((135, 90)), transforms.ToTensor(), normalize])
...
def load_datalist(leftdir, rightdir):
    ...
    egfr_dir = os.path.join(leftdir, 'egfr')
    ...
    no_egfr_dir = os.path.join(leftdir, 'egfr_no')
    ...
    right_egfr_dir = os.path.join(rightdir, 'egfr')
    right_egfr_folders = os.listdir(right_egfr_dir)
    for right_egfr_folder in right_egfr_folders:
        right_egfr_files = os.listdir(os.path.join(right_egfr_dir, right_egfr_folder))
        right_egfr_files.sort()
        one_patient_images = []
        for right_egfr_file in right_egfr_files:
            one_patient_images.append(os.path.join(right_egfr_dir, right_egfr_folder, right_egfr_file))
        image_list.append(one_patient_images)
        label_list.append(1)
        name_list.append(right_egfr_folder)
        # all_list.append([right_egfr_folder, one_patient_images, 1])

    right_no_egfr_dir = os.path.join(rightdir, 'egfr_no')
    right_no_egfr_folders = os.listdir(right_no_egfr_dir)
    for right_no_egfr_folder in right_no_egfr_folders:
        right_no_egfr_files = os.listdir(os.path.join(right_no_egfr_dir, right_no_egfr_folder))
        right_no_egfr_files.sort()
        one_patient_images = []
        for right_no_egfr_file in right_no_egfr_files:
            one_patient_images.append(os.path.join(right_no_egfr_dir, right_no_egfr_folder, right_no_egfr_file))
        image_list.append(one_patient_images)
        label_list.append(0)
        name_list.append(right_no_egfr_folder)
        # all_list.append([right_no_egfr_folder, one_patient_images, 0])
...
def loaderloader(leftdir, rightdir, batch_size):
    image_list, label_list, name_list, all_list = load_datalist(leftdir, rightdir)
...
if __name__ == '__main__':
    ...
    leftdir = '/disk1/liuzy/dataset/guizhou_dataset/2_split_all_dataset_by_slices_nums/feichuang_all/9_better_adaptive_z90_left_and_right/3_CT_multiply_Mask/split_left_right/0_255/left/dataset_z/1'
    rightdir = '/disk1/liuzy/dataset/guizhou_dataset/2_split_all_dataset_by_slices_nums/feichuang_all/9_better_adaptive_z90_left_and_right/3_CT_multiply_Mask/split_left_right/0_255/right/dataset_z/1'

    train_loader1, test_loader1, train_loader2, test_loader2, train_loader3, test_loader3, train_loader4, test_loader4, train_loader5, test_loader5 = loaderloader(leftdir, rightdir, 6)
    print('test_loader1')
