...
if __name__ == "__main__":
    ...
    plt.plot(mean_fpr,mean_tpr, color='g',label=r'DenseNet(2D)(area=%0.3f)'%mean_auc,lw=2,alpha=.8)
    ##########
    model_dic = 'model3_7_15_SVM_64_model_noNormalize'
    mean_fpr, mean_tpr, mean_auc = Five_FOld_MeanAUC(path, model_dic)
    plt.plot(mean_fpr, mean_tpr, color='b', label=r'CNN+SVM(2D)(area=%0.3f)' % mean_auc, lw=2, alpha=.8)
    ##########
    model_dic = 'model12_7_14_VGG16_64_model'
    mean_fpr, mean_tpr, mean_auc = Five_FOld_MeanAUC(path, model_dic)
    plt.plot(mean_fpr, mean_tpr, color='c', label=r'VGG16(2D)(area=%0.3f)' % mean_auc, lw=2, alpha=.8)
    ##########
    model_dic = "model15_11_12_SEResnet2D_64_model"
    mean_fpr, mean_tpr, mean_auc = Five_FOld_MeanAUC(path, model_dic)
    plt.plot(mean_fpr, mean_tpr, color='m', label=r'SEResNet(2D)(area=%0.3f)' % mean_auc, lw=2, alpha=.8)
    ##########
    model_dic = "model9_11_12_Resnet2D_151_transferlearning_model"
    mean_fpr, mean_tpr, mean_auc = Five_FOld_MeanAUC(path, model_dic)
    plt.plot(mean_fpr, mean_tpr, color='y', label=r'ResNet101(2D)(area=%0.3f)' % mean_auc, lw=2, alpha=.8)
    ##########
    model_dic = 'model9_11_12_Resnet_3D_41_transferlearning_model'
    mean_fpr, mean_tpr, mean_auc = Five_FOld_MeanAUC(path, model_dic)
    plt.plot(mean_fpr, mean_tpr, color='k', label=r'ResNet101(3D)(area=%0.3f)' % mean_auc, lw=2, alpha=.8)
    ##########
    model_dic = 'model19_7_11_Resnet1013D_64_model_mixup'
    mean_fpr, mean_tpr, mean_auc = Five_FOld_MeanAUC(path, model_dic)
    plt.plot(mean_fpr, mean_tpr, label=r'ResNet101(3D)(area=%0.3f)' % mean_auc, lw=2, alpha=.8)
    ##########
    model_dic = 'model16_7_14_Densenet3D_128_model_noNormalize'
    mean_fpr, mean_tpr, mean_auc = Five_FOld_MeanAUC(path, model_dic)
    plt.plot(mean_fpr, mean_tpr, label=r'DenseNet(3D)(area=%0.3f)' % mean_auc, lw=2, alpha=.8)
    ##########
