...
def draw_all(res_dir_list, color_list):
    plt.plot([0, 1], [0, 1], linestyle='--', lw=2, color='gray', label='Luck', alpha=.8)

    for i, res_dir in enumerate(res_dir_list):
        mean_fpr, mean_tpr, mean_auc = draw_roc(res_dir)
        plt.plot(mean_fpr, mean_tpr, color=color_list[i], label=r'Mean ROC (area=%0.2f)' % mean_auc, lw=2, alpha=.8)
