...
from network.MLP import Linear
...
################# Checkpoints
parser.add_argument('-c', '--checkpoint', default="/data2/lwy/projevt/Lung/VGG_egfr9_1_Dialated_AC_in_Dense_K_fold/output_clinical/4_6_MLP_clinicaldata", type=str, metavar='PATH',
...
def main1(train_loader1, test_loader1):
    ###########    model
    # model1 = Linear(19, 15, 10, 2, dropout_p=args.drop)
    model1 = Linear(19, 76, 38, 2, dropout_p=args.drop)
    model1.cuda()
...
def main2(train_loader2, test_loader2):
    ###########    model
    model2 = Linear(19, 76 , 38 , 2, dropout_p=args.drop)
    model2.cuda()
...
def main3(train_loader3, test_loader3):
    ###########    model
    model3 = Linear(19, 76 , 38 , 2, dropout_p=args.drop)
    model3.cuda()
...
def main4(train_loader4, test_loader4):
    ###########    model
    model4 = Linear(19, 76 , 38 , 2, dropout_p=args.drop)
    model4.cuda()
...
def main5(train_loader5, test_loader5):
    ###########    model
    model5 = Linear(19, 76 , 38 , 2, dropout_p=args.drop)
    model5.cuda()
