...
def p_all_CTs(Nodule_dir):
    Nodule_folders = os.listdir(Nodule_dir)
    res = {}
    for Nodule_folder in Nodule_folders:
        indexes = os.listdir(os.path.join(Nodule_dir, Nodule_folder, 'z'))
        indexes.sort()
        selected_index = '000.png'
        max_count = 0
        for index in indexes:
            image = cv2.imread(os.path.join(Nodule_dir, Nodule_folder, 'z', index))
            array = image[:, :, 0]
...
def heatmap_image(heatmap_test, image, slice, label, save, name):
    img = cv2.imread(os.path.join(image, slice))
    heatmap_test = cv2.resize(heatmap_test, (img.shape[1], img.shape[0]))
    ...
    if label[0]==label[1]:
        if label[1]==1:
            ...
            cv2.imwrite(os.path.join(save_path, name+'_'+slice), superimposed_img_test)
        elif label[1]==0:
...
if __name__=='__main__':
    # data_dir = '/disk1/liuzy/dataset/guizhou_dataset/all_new/Dataset/CT/dataset2_second_hu/0_255'
    data_dir = '/disk1/liuzy/dataset/guizhou_dataset/all_new/Dataset_112_112_112/CT/dataset1_second_hu/0_255'
    # checkpoint_PATH = '/disk1/liuzy/lung_cancer_project/VGG_egfr9_1_Dialated_AC_in_Dense_K_fold/output_ablation/Dialated_AC_in_DenseNet9_(CT2_second_hu_0_255)_(utils_acc_auc)_5fold_bs6_1/2/model_best.pth.tar'
    ...
    save_dir = '/disk1/liuzy/lung_cancer_project/VGG_egfr9_1_Dialated_AC_in_Dense_K_fold/visualize/res/112_112_112/test3'

    # dataloader
    all_loader = dataset.initialize_datasets(data_dir)

    # slice_selected
    slices = p_all_CTs('/disk1/liuzy/dataset/guizhou_dataset/all_new/Visualization/90_112_112/Nodule_mask')
