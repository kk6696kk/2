...
class DenseNet(nn.Module):
    def __init__(self, nb_filter=8, nb_block=2):
        super(DenseNet, self).__init__()

        self.pre = nn.Sequential(
            nn.Conv3d(1, nb_filter, kernel_size=3, stride=1, padding=1, dilation=2),
            nn.ELU(),)
        ...
        self.FC_layers = nn.Sequential(nn.Linear(3528, 1024, bias=True),
                                       nn.ReLU(),
                                       nn.Dropout(0.5),
...
if __name__=='__main__':
    ...
    datas = []
    data1 = torch.rand(11, 1, 10, 64, 64)
    datas.append(data1)
    for data in datas:
        print(data.size())
        output = model(data)
